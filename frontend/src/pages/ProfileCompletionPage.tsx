...
type SelfRegisterForm = UseFormReturn<SelfRegisterFormData>;

const selfRegisterDefaults: SelfRegisterFormData = {
  full_name: '',
  document_type: 'CPF',
  document_number: '',
  nationality: 'BRA',
...
function useSelfRegister(form: SelfRegisterForm) {
  ...
  async function onSubmit(data: SelfRegisterFormData) {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Erro ao realizar cadastro');
    } finally {
      setIsSubmitting(false);
    }
  }

  return { isSubmitting, submitError, success, campuses, onSubmit };
}

function CampusSelectField({ form, campuses }: { form: SelfRegisterForm; campuses: Campus[] }) {
  if (campuses.length <= 1) return null;
  return (
    <div className="space-y-2">
      <label htmlFor="campus_id" className="text-sm font-medium text-gray-900">
        Campus *
      </label>
      <select
        id="campus_id"
      ...
      {form.formState.errors.campus_id && (
        <p className="text-xs text-red-600">{form.formState.errors.campus_id.message}</p>
      )}
    </div>
  );
}

function RoleSelectField({ form }: { form: SelfRegisterForm }) {
  return (
    <div className="space-y-4 border-t pt-4">
      <h3 className="text-sm font-medium text-gray-900">Função na Organização *</h3>
      ...
      {form.formState.errors.role_type && (
        <p className="text-xs text-red-600">{form.formState.errors.role_type.message}</p>
      )}
    </div>
  );
}
...
export function ProfileCompletionPage() {
  ...
  const form = useForm<SelfRegisterFormData>({
    ...
    mode: 'onBlur',
    defaultValues: selfRegisterDefaults,
  });

  const { isSubmitting, submitError, success, campuses, onSubmit } = useSelfRegister(form);
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-2xl rounded-lg border border-gray-200 bg-white p-6 shadow-lg">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <CampusSelectField form={form} campuses={campuses} />
