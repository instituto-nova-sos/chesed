...
- **Public routes (Sprint 11, S12.1)**: the unauthenticated `/public/*` endpoints take the opposite stance from the pre-campus routes — instead of `BypassRLS` on the owner pool, they run on the **non-owner `chesed_app` pool inside a `PublicCampusTx`** whose `app.current_campus` GUC is set from a **validated** `campus_id` (the campus must exist and be active, checked against the RLS-exempt `campus` table before the transaction opens). Because the public write path is unauthenticated and internet-facing, RLS is kept as a fail-closed safety net: a handler bug attempting to write a different `campus_id` is rejected by the policy `WITH CHECK`, so a public request can never cross campuses. No schema change is required (the public path reuses the existing `person`/`person_role`/`volunteer_agreement`/`campaign` tables and their policies).
